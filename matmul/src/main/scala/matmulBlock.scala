...
@df class matmulBlockDriver extends DFSimDesign {
  ...
  sim.assert(a.partitions.map(_.address).allAreEqual, msg"unexpected partition mismatch", sim.Failure)
  sim.assert(a.partitions.map(_.ce).allAreEqual, msg"unexpected partition mismatch", sim.Failure)
  sim.assert(b.partitions.map(_.address).allAreEqual, msg"unexpected partition mismatch", sim.Failure)
  sim.assert(b.partitions.map(_.ce).allAreEqual, msg"unexpected partition mismatch", sim.Failure)
...
object Sim extends App {
  ...
  top_test.printCodeString
    .compile.printCodeString.toFolder("sim")
    .simulation.run()
}
...
object Synth extends App {
  ...
  top.printCodeString.compile.printCodeString
  .toFile("matmulBlock/zedboard/matmul_ait/xilinx/HLS/matmulBlock/solution1/impl/ip/hdl/vhdl/matmulBlock_moved.vhd")
}
